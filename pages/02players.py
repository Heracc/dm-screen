...
with st.expander("Add a Player"):
    with st.form("add_player", clear_on_submit=True, enter_to_submit=False):
        ...
        with st.container(horizontal=True):
            st.number_input("STR", placeholder="STR", min_value=0, step=1, width=100, key="strength_input")
            st.number_input("DEX", placeholder="DEX", min_value=0, step=1, width=100, key="dex_input")
            st.number_input("CON", placeholder="CON", min_value=0, step=1, width=100, key="con_input")
            st.number_input("INT", placeholder="INT", min_value=0, step=1, width=100, key="int_input")
            st.number_input("WIS", placeholder="WIS", min_value=0, step=1, width=100, key="wis_input")
            st.number_input("CHA", placeholder="CHA", min_value=0, step=1, width=100, key="cha_input")
